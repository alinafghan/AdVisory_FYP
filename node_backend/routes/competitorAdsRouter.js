...
const APIFY_TOKEN = process.env.APIFY_API_TOKEN3;
...
router.post('/scrape-facebook-ads', async (req, res) => {
  ...
  try {
    ...
    for (const item of items) {
      ...
      } else if (pageCategories.some(c => /movie|book|film|书籍|書籍|剧集|电影|娱乐网站/i.test(c))) {
      ...
      if (images.length > 0) {
        console.log("First few images:", JSON.stringify(images.slice(0, 2), null, 2));
      }
      ...
      if (!processedImages.length) {
        console.log(`⚠️ No images could be processed for ${pageName}, skipping ad`);
        continue;
      }
